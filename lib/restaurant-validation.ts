...
export function restaurantValidationError(input: Record<string, unknown> | null, step?: number): { message: string; step: number } | null {
  ...
  if (step === undefined || step === 0) {
    ...
    if (typeof input.slug !== "string" || input.slug.length > 80 || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(input.slug)) return { message: "Use lowercase letters, numbers, and single hyphens in the storefront address (up to 80 characters).", step: 0 };
  ...
  if ((step === undefined || step === 1) && !["saffron", "olive"].includes(String(input.theme))) return { message: "Choose a restaurant theme.", step: 1 };
  if (step === undefined || step === 2) {
    if (!text(input.dishName, 1, 100)) return { message: "Enter a dish name between 1 and 100 characters.", step: 2 };
    if (typeof input.dishPrice !== "number" || !Number.isInteger(input.dishPrice) || input.dishPrice < 1 || input.dishPrice > 100000) return { message: "Enter a whole-rupee dish price between ₹1 and ₹100,000.", step: 2 };
  }
  return null;
}
